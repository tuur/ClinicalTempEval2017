...
class Tokenization:
	...
	def get_vocabulary(self):
		self.vocabulary = Counter([re.sub(r'\d' ,'5',t.string) for t in self.tokens])
		return self.vocabulary


	def adaptation_unk(self, vocabulary, unk_threshold=1):
		unk_token = 'UNK'
		
		if not unk_token in vocabulary:
			vocabulary[unk_token] = 0
		
		for t in self.tokens:
			t.string = re.sub(r'\d' ,'5',t.string)
			if not t.string in vocabulary:
				t.string = '<UNK'+t.pos+'>'
				
				vocabulary[unk_token] += 1
			elif vocabulary[t.string] <= unk_threshold:
				del vocabulary[t.string]
				t.string = '<UNK'+t.pos+'>'
				vocabulary[unk_token] += 1
	...
	def adaptation_replace(self, vocabulary, embeddings):
		for t in self.tokens:
			
			if not t.string in vocabulary and t.string in embeddings.vocab:
				alternative = find_alternative(t.string, vocabulary, embeddings)
				if alternative:
	...
	def POS_tag(self):
		...
		if not standford_pos_tagger:
			...
			standford_pos_tagger = StanfordPOSTagger(os.getcwd() + '/english-bidirectional-distsim.tagger',os.getcwd() + '/stanford-postagger.jar')
			standford_pos_tagger.java_options = '-Xmx2g'
		...
		try:
			...
			standford_pos_tagger.java_options = '-Xmx4g'
			tags = standford_pos_tagger.tag([t.get_string() for t in selected_tokens])
			standford_pos_tagger.java_options = '-Xmx2g'
			print('needed increased RAM...')
	...
	def read_ctakes(self, doc_id, ctakes_out_dir):
		...
		# transformging ctakes ids to tokenization ids (token indices)
...
def test():
	...
	text = "Over de vroege geschiedenis van Rome is vrijwel niets met zekerheid bekend."
...
def find_alternative(word, candidates, embeddings, topn=10):
		similarity_list = embeddings.most_similar(positive=[word], negative=[],topn=topn)
		found = False
		for (c,s) in similarity_list:
			if not found and c in candidates:
