					...
					if e.next_entity and e.next_entity in entities and e.next_entity.paragraph == e.paragraph:
						for l1 in self.labels_ee:
							for l2 in self.labels_ee:
								#if l1!='no_label' and l2!='no_label':
...
class EntityFeatureHandler(FeatureHandler):
	entity_tokens = True
	...
	def extract(self, entity, document, update=True):
		...
		if self.entity_tokens:
			if self.conflate_digits:
				features.update({'entity_tokens:' + re.sub(r'\d','5',token.get_string()):1 for token in entity.tokens})
				features.update({'entity_string:' + re.sub(r'\d','5', str(entity)):1})
...
class DocTimeRelFeatureHandler(EntityFeatureHandler):
	entity_tokens = True
	entity_attributes = True
	entity_pos_tags = True
	character_n_grams = None
...
class EventFeatureHandler(EntityFeatureHandler):
	entity_tokens = True
	entity_attributes = False
	entity_pos_tags = True
	character_n_grams = None
	entity_tok_clusters=False
	label_bias = False
	...
	surrounding_entities = False
	surrounding_entities_pos = False

	closest_entity = False
	closest_verb = False
	
class Timex3FeatureHandler(EntityFeatureHandler):
	entity_tokens = True
	entity_attributes = False
	entity_pos_tags = True
	character_n_grams = None
