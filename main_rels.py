...
from thyme import read_thyme_documents,write_to_anafora, read_txt_corpus
...
parser.add_argument('-docs', type=int,default=1000000,
                    help='maximum number of documents used in each set (train, test)')
...
parser.add_argument('-local_initialization', type=int, default=0,
...
parser.add_argument('-negative_subsampling', type=str, default=None,
                    help='Type of negative sub-sampling used:close or random (default:None)')
...
parser.add_argument('-ctakes_out_dir', type=str, default='None',
...
parser.add_argument('-save_model', type=str,
                    help='saves the SP model to a file')																				
...
parser.add_argument('-unk_token', type=int, default=1,
                    help='Use UNK token for words with counts lower or equal than X (default:1)')	
...
if args.test_model:
	...
	labels_ee = set(args.tlinks.split(',') + ['no_label']) #set(['no_label','CONTAINS','BEFORE','OVERLAP'])#,'BEFORE','OVERLAP'])#,'BEGINS-ON','ENDS-ON','no_label'])
...
if args.load_document_structure:
	...
	thyme_document_structure = read_thyme_documents(args.thyme, regex='.*Temp.*', max_documents=args.docs, closure=closure, lowercase=bool(args.lowercase), conflate_digits=bool(args.conflate_digits), ctakes_out_dir=args.ctakes_out_dir if args.ctakes_out_dir!='None' else None,pos=args.pos, less_strict=False)	
...
constraints = set(args.constraints.split(',')) # C-trans and B-trans can make prediction slow!
...
if args.load_features and not args.test_model:
	...
	X, Y = [], []
	for i,doc in enumerate(thyme_document_structure[:args.docs]):
...
if structured_prediction and not args.test_model:
	...
	if args.train_model:
		print('saving model to',args.train_model+'/tlinks.p')
		SP.save_model(args.train_model+'/tlinks.p')
...
if structured_prediction:
	...
	print('loss_e:',np.mean([SP.loss_both(Ys_p[i], Y[i])[0] for i in range(len(Y))]),'loss_ee:',np.mean([SP.loss_both(Ys_p[i], Y[i])[1] for i in range(len(Y))]))
	if args.test_xml:
		evaluation = Evaluation(Ys_p, Y, name='GLOBAL LEARNING', tasks=args.tasks)
	
if (local_prediction or local_initialization) and not args.test_model:
	Yl_p = []
	for xe,xee in X:
		if 'TLINK' in args.tasks.split(','):
...
if args.output_xml_dir:
	...
	write_to_anafora(X, preds, args.output_xml_dir, thyme_document_structure)	
